...
def remove_punct(text):
    ...
    """remove_punct = disct.fromkeys(map(ord, string.punctuation))
    return(text.translate(remove_punct))"""
    i = ''
    for x in range(len(text)):
        if text[x].isalpha() or text[x] == ' ':
            i += str(text[x])
    return i
...
def normalise_input(user_input):
    ...
    'help'
    """
    """user_input.translate(None, string.punctuation)
    user_input.lower()
    user_input.strip()"""
    
    remove_punctuation = dict.fromkeys(map(ord, string.punctuation))
    return(user_input.translate(remove_punctuation).lower().strip())
